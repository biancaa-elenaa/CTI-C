...
Lista_t makeListafromArray(Payload_t *v,int n)
{
    Lista_t L=makeLista();
    for(int i=0;i<n;i++)
    {
...
Payload_t *makeArrayfromLista(Lista_t L,int *nr_elem)
{   
    for(Nod_t *c=L;c!=NULL;c=c->urm)
        (*nr_elem)++;
    Payload_t *v=NULL;
    v=malloc(sizeof(Payload_t) * (*nr_elem));
...
void printArray(Payload_t *v,int n)
{
    for(int i=0;i<n;i++)
    {
        printf("%u ", v[i]);
...
int main()
{
    ...
    for(int i=0;i<n;i++)
        v[i]=i*2;
    L=makeListafromArray(v,6);
    int size=0;
    Payload_t *vector = makeArrayfromLista(L,&size);
